...
if [[ -n "${OKJA_MODEL_SHA256:-}" && "$ACTUAL_ARCHIVE_SHA256" != "$OKJA_MODEL_SHA256" ]]; then
  echo "Zipformer archive SHA-256 mismatch" >&2
...
# Keep only the files used by sherpa's Korean streaming int8 config.
...
(
  cd "$DST"
  sha256sum encoder-epoch-99-avg-1.int8.onnx decoder-epoch-99-avg-1.onnx \
    joiner-epoch-99-avg-1.int8.onnx tokens.txt > OKJA_MODEL_FILES.sha256
)

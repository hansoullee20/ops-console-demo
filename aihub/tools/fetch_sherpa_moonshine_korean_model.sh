...
if [[ -n "${OKJA_MODEL_SHA256:-}" && "$ACTUAL_ARCHIVE_SHA256" != "$OKJA_MODEL_SHA256" ]]; then
  echo "Moonshine archive SHA-256 mismatch" >&2
  echo "expected: $OKJA_MODEL_SHA256" >&2
  echo "actual:   $ACTUAL_ARCHIVE_SHA256" >&2
  exit 1
fi

# Upstream does not currently provide a digest in the Okja repository. Record the
# fetched archive digest for benchmark provenance; release builds must use a reviewed
# pinned digest rather than treating this first-seen value as trust.
...
# Files referenced by sherpa-onnx getOfflineModelConfig(type=51).
cp "${SRC}/encoder_model.ort" "$DST/"
cp "${SRC}/decoder_model_merged.ort" "$DST/"
cp "${SRC}/tokens.txt" "$DST/"
cp provenance.txt "$DST/OKJA_MODEL_PROVENANCE.txt"

(
  cd "$DST"
  sha256sum encoder_model.ort decoder_model_merged.ort tokens.txt > OKJA_MODEL_FILES.sha256
)

...
app/
  ...
  services/import_watch.py  내보내기 폴더 감시 (미리보기까지만 자동)
...
**이 앱은 지문 단말에 직접 접속하지 않습니다.** 단말 관리 프로그램(PC)에서 해당 월을
조회한 뒤 **내보내기(Export)** 로 `.XLS` 파일을 먼저 내려받고, 그 파일을 앱에 올립니다.

```
(단말 프로그램) 월 조회 → 내보내기 → .XLS 저장
        ↓
(콘솔) 파일 선택 → 미리보기 → 확인 → 적용      (문제가 있으면 되돌리기)
```

그래서 지문 근태는 항상 **마지막 가져오기 시점 기준**입니다. 상단 배지와 `/health` 의
`fingerprint.last_import_at` 이 그 시각을 보여주며, 하루가 지나면 stale 로 표시됩니다.
...
단말 프로그램에 **예약 내보내기**가 있으면, 고정 폴더로 저장하도록 설정하고 그 폴더를
백엔드에 알려주십시오. 예약 기능이 없어도 담당자가 그 폴더에 파일만 떨궈두면 동일하게
동작합니다.
...
- 새 파일이 보이면 **미리보기까지만 자동**으로 진행하고 `확인 대기` 로 올려둡니다.
  상단 버튼에 `지문 XLS 가져오기 · 확인 대기 N` 으로 표시됩니다.
- **적용은 절대 자동으로 하지 않습니다.** 부분 export 나 단말 장애 중에 뽑힌 파일이
  자동 반영되면 그날은 전 직원 무기록으로 보이고, 아무도 그 숫자의 출처를 모릅니다.
- 쓰다 만 파일은 읽지 않습니다. 크기·수정시각이 두 번 연속 같아야 읽습니다.
- 같은 파일인지는 **이름이 아니라 내용 해시**로 판단합니다. 매달 같은 파일명으로
  내보내도 되고, 내용이 같은 재-export 는 건너뜁니다.
- **원본 파일은 옮기거나 지우지 않습니다.** 복사본만 `uploads/` 에 보관합니다.
- 감시 중인 폴더, 발견/실패는 백엔드 로그에 남습니다.

...
def list_leave(conn: sqlite3.Connection, year: int | None = None) -> list[dict]:
    where = "WHERE l.leave_year = ?" if year else ""
...
def correct_leave(conn: sqlite3.Connection, leave_id: int, *, employee_id: int,
    ...
    dates = set(work_calendar.working_dates(conn, old["start_date"], old["end_date"])) | set(work_calendar.working_dates(conn, start_date, end_date))
    _sync_attendance(conn, old["employee_id"], dates)
    if employee_id != old["employee_id"]: _sync_attendance(conn, employee_id, dates)
    return serialize(_get(conn, leave_id))
